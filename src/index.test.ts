...
import { AtomicAction, AtomicReducerFunc, createAtomic, REDUX_ATOMIC_ACTION, generateKey } from "./index";
...
describe("We're testing this approach", () => {
  ...
  it("Types are passed through", () => {
    let store = createStore(sampleApp);

    store.dispatch(actions1.changeTitle(0));

    const state: any = store.getState();
    expect(state.atomicOne.title).toEqual(0);
  });
});
