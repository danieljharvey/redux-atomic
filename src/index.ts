...
function createObjectExporter<s>(exporter: AtomicExporter<s>) {
  return function(obj: AtomicActionList<s>) {
    ...
    Object.keys(obj).forEach(key => {
      const func = obj[key];
      const dispatchFunc = createDispatchFunc(exporter, key, func);
      newObj[key] = dispatchFunc;
    });
    return newObj;
  };
}

function createDispatchFunc<s, T>(
  exporter: AtomicExporter<s>,
  key: string,
  func: AtomicDispatchFunc<s, T>
): AtomicReadyDispatchFunc<s, T> {
  return exporter(func, key);
}
...
export function createAtomicExporter<s, p>(key: object, reducerName: string) {
  return function wrapperMaker<s, T>(func: AtomicDispatchFunc<s, T>, funcName?: string) {
